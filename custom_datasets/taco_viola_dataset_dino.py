...
class TacoViolaDatasetDINO(Dataset):
    """
    ...
    def __init__(self, annotations_file, img_dir, transform=None, classes=None, mapping=None):
        super().__init__()
        ...
        # Print Mapping form TACO to Viola77 dataset
        print(f"Available classes: {mapping}") 
        self.mapping_taco2viola = mapping
        ...
        self.anns_ids = [
            ...
            if self.mapping_taco2viola[self.coco_data.loadAnns(ann_id)[0]['category_id']] != 999
        ]

        # Filter annotations based on box size
        for ann_id in self.anns_ids:
            annotation = self.coco_data.loadAnns(ann_id)[0]
            if annotation['bbox'][2] < 150 or annotation['bbox'][3] < 150:
                self.anns_ids.remove(ann_id)
        ...
        # Ensure classes are defined properly
        ...
        # Store the category ID → cluster category mapping
        self.mapping_taco2viola = mapping
        print(f"Mapping TACO to Viola: {self.mapping_taco2viola}")


    def __len__(self) -> None:
        """ Returns the length of the dataset """
        return len(self.anns_ids)  # <--- This is the number of annotations, not images
    ...
    def __getitem__(self, idx) -> dict:
        ...
        category_id = self.mapping_taco2viola[annotation['category_id']]
        ...
        squared_img = Image.fromarray(squared_img)
        ...
        # Convert category ID → cluster category name → numerical class index
        class_name = self.mapping_taco2viola.get(category_id, "Unknown")  # Ensure safe lookup
        class_idx = self.cluster_class_to_idx.get(class_name, len(self.classes))  # Assign "Unknown" class if needed
